...
class SafePasswordResetView(PasswordResetView):
    ...
    def form_valid(self, form):
        try:
            ...
            messages.error(self.request, "Could not send email right now. Please try again later or contact admin.")
...
def student_dashboard(request):
    ...
    return render(request, 'student_dashboard.html', {'requests': user_requests})
...
def hod_dashboard(request):
    ...
    return render(request, 'hod_dashboard.html', {'requests': pending_requests})
...
def approve_request(request, request_id):
    ...
    if request.method == "POST":
        ...
        certificate.save()
        return redirect('hod_dashboard')
    return render(request, 'approve_request.html', {'certificate': certificate})
...
def reject_request(request, request_id):
    ...
    if request.method == "POST":
        ...
        certificate.save()
        return redirect('hod_dashboard')
    return render(request, 'reject_request.html', {'certificate': certificate})
...
def principal_dashboard(request):
    ...
    return render(request, 'principal_dashboard.html', {'requests': requests})
...
def principal_approve(request, request_id):
    ...
    if request.method == "POST":
        ...
        certificate.save()
        return redirect('principal_dashboard')
    return render(request, 'principal_approve.html', {'certificate': certificate})
...
def principal_reject(request, request_id):
    ...
    if request.method == "POST":
        ...
        certificate.save()
        return redirect('principal_dashboard')
    return render(request, 'principal_reject.html', {'certificate': certificate})
...
def staff_dashboard(request):
    ...
    return render(request, 'admin_dashboard.html', {'requests': approved_requests})
...
def generate_certificate(certificate):
    ...
    p.drawCentredString(width / 2, height - 80, "BHARATA MATA COLLEGE (AUTONOMOUS)")
    p.setFont("Helvetica-Bold", 14)
    p.drawCentredString(width / 2, height - 105, "THRIKKAKARA")
    today = date.today().strftime("%d/%m/%Y")
    p.setFont("Helvetica", 11)
    p.drawRightString(width - 50, height - 130, f"Date: {today}")
    p.setFont("Helvetica-Bold", 16)
    p.drawCentredString(width / 2, height - 160, "CERTIFICATE")
    p.line(width/2 - 70, height - 165, width/2 + 70, height - 165)
    p.setFont("Helvetica", 12)
    text = p.beginText(80, height - 210)
    text.setLeading(25)
    text.textLine(f"This is to certify that {certificate.student.get_full_name() or certificate.student.username}")
    text.textLine(f"is a student of {certificate.student.get_department_display()} Department")
    text.textLine(f"for the academic year {certificate.created_at.year}-{certificate.created_at.year + 1}.")
    text.textLine("")
